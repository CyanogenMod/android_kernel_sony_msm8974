...
#ifdef GSCAN_SUPPORT
static
uint64 convert_fw_rel_time_to_systime(uint32 fw_ts_ms)
{
	struct timespec ts;
...
_dhd_pno_set(dhd_pub_t *dhd, const dhd_pno_params_t *pno_params, dhd_pno_mode_t mode)
{
	...
	if (mode & DHD_PNO_GSCAN_MODE) {
		...
		lost_network_timeout = (pno_params->params_gscan.max_ch_bucket_freq *
			pfn_param.scan_freq * pno_params->params_gscan.lost_ap_window);
...
static
wlc_ssid_ext_t * dhd_pno_get_legacy_pno_ssid(dhd_pub_t *dhd,
	dhd_pno_status_info_t *pno_state)
{
	int err = BCME_OK;
	int i;
...
void *
dhd_handle_hotlist_scan_evt(dhd_pub_t *dhd, const void *event_data, int *send_evt_bytes,
	hotlist_type_t type)
{
	void *ptr = NULL;
	dhd_pno_status_info_t *_pno_state = PNO_GET_PNOSTATE(dhd);
